  ...
  if (nrow(inData) == 1) {
    navsetCardTabSummary <- 
      htmltools::p(
        htmltools::HTML(
          paste0(
            "Chill accumulation at the AZMet ", azmetStation, " station from ", gsub(" 0", " ", format(startDate, "%B %d, %Y")), " through ", gsub(" 0", " ", format(endDate, "%B %d, %Y")), " is ", "<b>", format(round(currentYearTotal, digits = 2), nsmall = 2), " inches</b>."
          ),
        ),
    ...
    averageTotal <- mean(inData$chill_total, na.rm = TRUE)
